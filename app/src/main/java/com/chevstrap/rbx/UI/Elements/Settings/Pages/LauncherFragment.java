...
public class LauncherFragment extends Fragment {
	...
	private void initialize(Bundle _savedInstanceState, View _view) {
		...
		imageview_switch5.setOnClickListener(v -> {
			if (getStateToggleByImage(imageview_switch5)) {
				try {
					saveSet("ClearCacheEveryRBXLaunch", String.valueOf(false));
				} catch (JSONException ignored) {

				}
			} else {
				try {
					saveSet("ClearCacheEveryRBXLaunch", String.valueOf(true));
				} catch (JSONException ignored) {
	...
	private void initializeLogic() {
		...
		int selectedIndex = 0; // default to "Automatic"
		...
		// Set toggle state
		setStateToggleByImage(imageview_switch5, getStateSettingKey("ClearCacheEveryRBXLaunch"));
	}
